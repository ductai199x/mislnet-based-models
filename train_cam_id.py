...
def prepare_datasets(args: dict[str, Any]) -> Tuple[DataLoader, DataLoader]:
    train_ds = CamIdDataset(
        root_dir=args["root_dir"],
        ds_patch_size=args["data_args"]["patch_size"],
        model_patch_size=args["model_args"]["patch_size"],
        split="train",
    ...
    args["model_args"]["num_classes"] = len(train_ds.label_map)
...
def parse_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    args.model_args = config.config["model_args"]
    args.data_args = config.config["data_args"]
    args.training_args = config.config["training_args"]
